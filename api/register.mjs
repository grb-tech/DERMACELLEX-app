...
//
// 2026-09-08: md 문서 3.3절 "기존 고객 흐름" 대응 — body.existing에 기존 거래처 · 담당자
// ID가 오면(08 화면에서 이미 이메일로 식별됨) 새로 만들지 않고 그대로 재사용하며, 새 제조
// 문의만 "기존" 구분으로 생성한다. 진단은 이 흐름에서 아예 호출되지 않는다(md 4번 항목).
...
export default async function handler(req, res) {
  ...
  try {
    ...
    let clientId, contactId, businessName;

    if (existing?.clientId && existing?.contactId) {
      // ─── 기존 고객: 거래처 · 담당자를 새로 만들지 않고 그대로 재사용 ───
      clientId = existing.clientId;
      contactId = existing.contactId;
      businessName = existing.businessName || '고객';
    } else {
    ...
    const inquiryPage = await createPage(TOKEN, DB.INQUIRY, {
      '제조 문의명': title(`[${inquiryUid}] ${businessName} | 제조개발 문의`),
      '고유 ID': text(inquiryUid),
      '고객구분': select(existing ? '기존' : '신규'),
      '문의유형': select(existing ? '재문의' : '신규개발'),
      '상태': { status: { name: '접수' } },
